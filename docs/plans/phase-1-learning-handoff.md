...
3. **P1-01 through P1-08 are complete.** The bounded P1-04 discovery run has
   ...
   `covered`, and chunking/citation `gap`. Review the **approved manifest** (`manifests/phase1-discovery-v1.json`, local-only evidence omitted from Git),
   **decision record** (`manifests/phase1-discovery-v1-review.json`, local-only evidence omitted from Git),
   **screening rationale** (`manifests/phase1-discovery-v1-screening-proposal.md`, local-only evidence omitted from Git),
   and **yield/coverage report** (`manifests/phase1-discovery-v1-report.json`, local-only evidence omitted from Git).
   ...
   prefilter. The user approved the **ten-paper sample** (`manifests/phase1-discovery-v1-pdf-reference-proposal.md`, local-only evidence omitted from Git)
   ...
   **acquisition inventory** (`manifests/phase1-discovery-v1-pdf-acquisition.json`, local-only evidence omitted from Git).
   Use the **compact PDF review index** (`manifests/phase1-discovery-v1-pdf-review.md`, local-only evidence omitted from Git).
   ...
   for Phase 2. Review the **local sample packet** (`local-reference/phase1-discovery-v1/review-draft.md`, local-only evidence omitted from Git)
   ...
   records the verified outcomes; the hosted CI and snapshot-finalization gates are complete.
4. **P1-14 status:** the assistant accepted the **100-title membership** (`manifests/phase1-100-paper-membership-decision.json`, local-only evidence omitted from Git) on 2026-09-25 under the user's explicit delegation. The set is 10 v1 papers plus 21 expansion, 51 cited-work and 18 cache-screen candidates; the approved v1 remains unchanged. All 100 selected PDFs are acquired, checksum-valid and linked to persisted storage/index permission evidence. The **source-review closeout** (`local-reference/phase1-100/source-review-closeout.json`, local-only evidence omitted from Git) resolves the three checks listed in the initial source-route review. The [100-paper acceptance evidence report](../reference/phase-1-100-paper-acceptance-report.md) records extraction, table review, recovery and index reconciliation. Six privately stored follow-up PDFs remain unassociated and do not count toward acceptance. No further copyright decision is pending. Snapshot `4b11fab3-d4a5-4e7a-a58e-8654accf2c6c` was finalized after hosted CI run 36231611600 passed on revision `ed54a046429a7b288830eca0d1a1bcf4e3de87cf`. The current worktree passed 188 local tests, including all 15 live service checks; Ruff check/format (92 files), strict mypy (40 source files), migration, `pip check`, `pip-audit` and Linux AMD64 Docker build also passed.
...
- Preserve applied `001_initial.sql`. Migrations `002` through `014` add the
  Phase 1 schema, including the persisted full document plan and terminal checkpoints; the runner applies each migration and its version atomically
...
Current status: P1-01 through P1-12 are complete for the ten-paper workflow. The delegated 100-paper membership is recorded, all 100 selected PDFs passed exact source/checksum/permission review, and the 100-paper extraction completed for every member. The manual sample covers 83 flagged tables and 20 ordinary-sample packet items; one item was reclassified as a figure, and all 269 sampled unique numeric values were present after source-linked corrections. Snapshot validation in `research_phase1_review` reports no issues, and snapshot-filtered PostgreSQL/Qdrant evidence IDs match (44,277). The default `research` database does not contain this snapshot; point `RESEARCH_PLATFORM_DATABASE_URL` at the review database for its CLI commands. The [100-paper acceptance report](../reference/phase-1-100-paper-acceptance-report.md) and **source closeout** (`local-reference/phase1-100/source-review-closeout.json`, local-only evidence omitted from Git) record these results. Current-worktree local checks passed 188 tests in 3.59 seconds, including all 15 live service checks; Ruff check/format (92 files), strict mypy (40 source files), migration, `pip check`, `pip-audit` and the Linux AMD64 Docker build also passed. Hosted CI run [36231611600](https://github.com/avsngh-git/RAGpipeline/actions/runs/36231611600) passed on revision `ed54a046429a7b288830eca0d1a1bcf4e3de87cf`; the 100-paper snapshot is finalized, validates with no issues, and P1-14 is complete. Six additional private follow-up PDFs remain unassociated and outside the accepted set.

Astra’s 2026-09-26 completion audit is recorded in [the audit](../reviews/phase-1-completion-audit-2026-09-26.md). The six findings are corrected in the worktree; the audit’s local verification and remaining hosted-CI gate are recorded there.
...
See **the manifest** (`manifests/phase1-discovery-v1.json`, local-only evidence omitted from Git),
**review decisions** (`manifests/phase1-discovery-v1-review.json`, local-only evidence omitted from Git),
**screening rationale** (`manifests/phase1-discovery-v1-screening-proposal.md`, local-only evidence omitted from Git),
**the report** (`manifests/phase1-discovery-v1-report.json`, local-only evidence omitted from Git), and the
...
The **inventory** (`manifests/phase1-discovery-v1-pdf-acquisition.json`, local-only evidence omitted from Git) records
...
parser outputs are stored in ignored **local-reference data** (`local-reference/phase1-discovery-v1/review-draft.md`, local-only evidence omitted from Git).
